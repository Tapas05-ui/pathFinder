...
export default function Onboarding() {
  ...
  const initConversation = async () => {
    ...
    try {
      ...
      if (data.messages?.length > 0) { setMessages(data.messages); setResumed(true); }
      else await startFresh();
    } catch { await startFresh(); }
    finally { setInitializing(false); setTimeout(() => inputRef.current?.focus(), 150); }
  };

  const startFresh = async () => {
    setLoading(true);
    try {
      const { data } = await api.post("/chat/onboarding", {});
  ...
  const sendMessage = async () => {
    ...
    if (userMsg.toLowerCase() === "generate") { await handleGenerate(); return; }

    try {
      const { data } = await api.post("/chat/onboarding", { message: userMsg });
  ...
  const handleGenerate = async () => {
    setGenerating(true);
    setMessages((p) => [...p, { role: "assistant", content: "✨ Perfect! Analysing everything you've shared and building your personalized career roadmap. This will take around 15 seconds..." }]);
    try {
      await api.post("/roadmap/generate");
      setTimeout(() => navigate("/dashboard"), 2500);
    } catch {
      setMessages((p) => [...p, { role: "assistant", content: "Oops! Something went wrong. Type 'generate' again to retry." }]);
      setGenerating(false);
    } finally { setLoading(false); }
  };

  const placeholder = initializing ? "Loading..." : generating ? "Generating roadmap..." : loading ? "PathFinder is typing..." : 'Your answer... (Enter to send)';

  if (initializing) return (
    <div className="min-h-screen bg-midnight flex items-center justify-center">
      <div className="text-center fade-up">
        <div className="w-10 h-10 border-2 border-accent border-t-transparent rounded-full animate-spin mx-auto mb-4" />
        <p className="text-soft text-sm font-body">Loading your conversation...</p>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-midnight flex flex-col">
      ...
      <div className="border-b border-border/60 px-6 py-4 flex items-center justify-between bg-midnight/80 backdrop-blur-sm sticky top-0 z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-display font-bold text-white text-sm">PathFinder AI</p>
            <div className="flex items-center gap-1.5 mt-0.5">
              <span className="w-1.5 h-1.5 bg-accent rounded-full animate-pulse" />
              <span className="text-soft text-xs font-body">{generating ? "Building your roadmap..." : "Getting to know you..."}</span>
            </div>
          </div>
        </div>
        <div className="text-right">
          <p className="text-soft text-sm font-body">Hey, {user?.name?.split(" ")[0]} 👋</p>
          {messages.length >= 12 && !generating && (
            <p className="text-accent text-xs font-body mt-0.5 animate-pulse">Type <span className="font-semibold">"generate"</span> when ready ✨</p>
          )}
        </div>
      </div>
      ...
      {resumed && (
        <div className="bg-accent/10 border-b border-accent/20 px-6 py-2.5 text-center fade-in">
          <p className="text-accent text-xs font-body">👋 Welcome back! Resuming your conversation where you left off.</p>
        </div>
      )}
      ...
      <div className="border-t border-border/60 px-4 py-4 bg-midnight/80 backdrop-blur-sm">
        <div className="max-w-2xl mx-auto flex gap-3">
          ...
          <button onClick={sendMessage} disabled={loading || generating || initializing || !input.trim()}
