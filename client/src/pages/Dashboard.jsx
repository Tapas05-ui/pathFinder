...
export default function Dashboard() {
  ...
  useEffect(() => {
    Promise.all([fetchRoadmap(), fetchCheckinStatus()]).finally(() =>
      setLoading(false),
    );
  }, []);
  ...
  const fetchCheckinStatus = async () => {
    try {
      const { data } = await api.get("/api/checkin/status");
  ...
  const handleTabSwitch = async (index) => {
    ...
    setActiveTab(index);
    try {
      const { data } = await api.patch("/api/roadmap/active-path", {
        pathIndex: index,
      });
      setRoadmap(data);
    } catch (err) {
  ...
  const updateMilestoneLocally = (pathIndex, milestoneIndex, completed) => {
    setRoadmap((prev) => {
      ...
      updated.overallProgress = Math.round(
        (done / path.milestones.length) * 100,
      );
      return updated;
    });
  };
  ...
  const completeMilestone = async (pathIndex, milestoneIndex) => {
    ...
    try {
      const { data } = await api.patch(
        `/api/roadmap/milestone/${pathIndex}/${milestoneIndex}/complete`,
      );
      setRoadmap(data);
  ...
  const undoMilestone = async (pathIndex, milestoneIndex) => {
    ...
    try {
      const { data } = await api.patch(
        `/api/roadmap/milestone/${pathIndex}/${milestoneIndex}/undo`,
      );
      setRoadmap(data);
    } catch {
      fetchRoadmap();
  ...
  const completedCount =
    activePath?.milestones.filter((m) => m.completed).length || 0;
  ...
  return (
    <div className="min-h-screen bg-midnight">
      ...
      <div className="max-w-3xl mx-auto px-4 py-8 space-y-5">
        ...
        {/* Progress */}
        <ProgressCard
          careerTitle={activePath?.title || "Your Career Path"}
          ...
          <div className="fade-up-4 space-y-3">
            <div className="flex items-center justify-between px-1">
              ...
              <span className="text-muted text-xs font-body">
                Click ✓ to complete · Click again to undo
              </span>
            </div>
        ...
        {completedCount === totalCount && totalCount > 0 && (
          <div className="fade-in bg-gradient-to-r from-accent/10 to-emerald-500/10 border border-accent/30 rounded-2xl p-8 text-center">
            ...
            <p className="text-soft text-sm font-body max-w-xs mx-auto">
              You've finished all milestones. Incredible work!
            </p>
          </div>
        )}
